...
def build_solver_prompt(matrix, true_number, k=3, row_order=None, col_order=None):
    assert row_order is not None and col_order is not None, \
        "Must pass row_order and col_order computed in main."
    ...
    # pick example indices different from target digit label (unchanged logic) ...
    import random
    ...
    # 🔒 Use the SAME permutation for both ICL examples:
    ...
    # The target matrix here should already be permuted in main,
    # but it's fine to be explicit:
    matrix_perm = apply_permutation(matrix, row_order, col_order) if matrix.shape != ex1_perm.shape else matrix
    prompt += f"Matrix:\n{matrix_perm.tolist()}"
