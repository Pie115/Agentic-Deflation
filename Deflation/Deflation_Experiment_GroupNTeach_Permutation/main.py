...
while (index < len(indexes)):
    ...
    # === GroupNTeach permutation (once per image) ===
    k_blocks = 4  # adjust if desired (e.g., 2 for 8x8 digits)
    img_perm, row_order, col_order, row_inv, col_inv, groups, D = groupnteach_permutation(
        img.astype(float), k=k_blocks, density_mode="mean", random_state=0
    )
    img_perm = img_perm.astype(int)

    # Save original permuted reference for deflation evaluator
    save_single_image(img_perm, "original_blocky_no_deflation.png")

    # Track full trajectory (both spaces)
    ...
    while (deflation_decision == 'accept'):
        # Build prompt using SAME permutation for ICL examples
        ...
        while (gemini_decision == 'reject'):
            ...
            # Qwen rank-1 evaluator on permuted space
        ...
        if deflation_decision == 'reject':
            # Either finished or bailed due to too many retries
        ...
        # Deflation evaluator compares original permuted vs current residual permuted
