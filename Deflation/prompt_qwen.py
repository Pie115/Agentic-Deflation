...
def build_messages_for_similarity(img_path_a, img_path_b):
    sys_rules = (
...
def build_messages_for_deflation(img_path_a, img_path_b):
    sys_rules = (
...
def _run_qwen(model, processor, messages, max_new_tokens=128):
...
def qwen_similarity_verdict(model, processor, img_path_a, img_path_b, max_new_tokens=128):
    messages = build_messages_for_similarity(img_path_a, img_path_b)
    output_text = _run_qwen(model, processor, messages, max_new_tokens=max_new_tokens)
...
def qwen_deflation_verdict(model, processor, img_path_a, img_path_b, max_new_tokens=128):
    messages = build_messages_for_deflation(img_path_a, img_path_b)
    output_text = _run_qwen(model, processor, messages, max_new_tokens=max_new_tokens)
...
def evaluate_pair_with_qwen(model, processor, img_path_true, img_path_gemini):
    verdict = qwen_similarity_verdict(model, processor, img_path_true, img_path_gemini)
    decision = "accept" if verdict["similar"] else "reject"
    reason = f"Evaluator (Qwen): {verdict['justification']}"
    return {
        "decision": decision,
        "reason": reason,
...
def evaluate_deflation_with_qwen(model, processor, img_path_true, img_path_gemini):
    verdict = qwen_deflation_verdict(model, processor, img_path_true, img_path_gemini)
    decision = "accept" if verdict["similar"] else "reject"
    reason = f"Evaluator (Qwen): {verdict['justification']}"
    return {
        "decision": decision,
